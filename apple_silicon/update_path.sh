...
if [[ $current_arch == "i386" ]]; then
    ...
    export PATH=`echo $TMP_PATH | tr ":" "\n" | grep -v "/opt/homebrew/bin" | grep -v "/opt/homebrew/sbin" | grep -v "/opt/homebrew/opt/llvm/bin" | tr "\n" ":"`

else
    TMP_PATH="/opt/homebrew/opt/llvm/bin:${PATH}"

    export PATH=`echo $TMP_PATH | tr ":" "\n" | grep -v "/opt/homebrew/bin" | grep -v "/opt/homebrew/sbin" | grep -v "/usr/local/opt/llvm/bin" | tr "\n" ":"`
